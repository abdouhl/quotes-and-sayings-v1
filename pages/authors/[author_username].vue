...
<div v-for="quote in quotes_list[0].body.filter(l => l['username'] == author_username )" class="flex flex-col rounded-lg bg-white divide-y  px-1 md:px-2">
<p class="flex-1 p-1 md:p-2 flex items-center"><a :href="localePath('/quotes/'+quote.key)" >{{quote.text}}</a></p>
<h2 class="flex-none text-xl text-center py-1 " style="font-family: Lobster, cursive;">{{quote.name}}</h2>
</div>
